...
function validate({ title, text, type, mode, urgency, location }) {
  ...
  if (type !== "Emergency") {
    ...
    if (![1, 2, 3].includes(Number(urgency))) {
      return "Urgența trebuie să fie 1, 2 sau 3.";
    }
  }
...
export default function Create() {
  ...
  const finalMode = isEmergency ? "need" : mode;
  const finalUrgency = isEmergency ? 3 : Number(urgency);
  ...
  }, [title, text, type, finalMode, finalUrgency, location, uid, submitting]);
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] w-full bg-zinc-950 text-zinc-100">
      <div className="mx-auto max-w-2xl px-4 py-5">
        <h1 className="mt-1 text-2xl font-extrabold">Creează o postare</h1>
        ...
        <form onSubmit={onSubmit} className="mt-5 space-y-4">
          <div className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {TYPES_BTN.map((t, i) => (
                <button
                  type="button"
                  key={t}
                  onClick={() => handleTypeChange(TYPES[i])}
            ...
            {isEmergency ? (
              <div className="mt-4 rounded-2xl border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-100">
                Pentru o urgență, aplicația setează automat postarea ca
                <span className="font-extrabold"> „Are nevoie” </span>
                și gradul ca
          ...
          <div className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-4">
            ...
            <input
              ...
              placeholder={
                isEmergency
                  ? 'Ex.: "Pană de curent pe scară"'
                  : 'Ex.: "Am nevoie de o scară"'
              }
              maxLength={80}
            />
            ...
            <textarea
              ...
              placeholder={
                isEmergency
                  ? "Descrie rapid ce se întâmplă și unde este problema."
                  : "Explică ce ai nevoie sau ce poți oferi."
              }
              maxLength={600}
            />
          ...
          >
            {submitting ? "Se postează..." : "Creează postarea"}
          </button>
        </form>
      </div>
    </div>
