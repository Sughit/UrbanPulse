...
function typeLabel(type) {
  return TYPE_LABELS[type] || type || "Postare";
}

function statusLabel(status) {
  return STATUS_LABELS[status] || status || "deschisă";
}
...
export default function Home() {
      ...
      try {
        ...
        if (severeAlert) {
          ...
          await ensureSafetyCheckin({
            eventKey,
            location: myPos,
            severeTitle: severeAlert.event || "Alertă meteo severă",
          });
  ...
  async function onSafetyCheckin() {
    ...
    try {
      ...
      alert(e?.message || "Nu am putut salva verificarea de siguranță.");
    } finally {
      setChecking(false);
    }
  }
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="text-sm font-extrabold text-blue-200">
                  Alertă meteo: {severeAlert.event || "Alertă meteo severă"}
                </div>
                <div className="mt-1 text-sm text-zinc-200">
                  Apasă „Sunt OK” ca să apari în verificarea de siguranță.
                </div>
              </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-zinc-500">
                          <span>Status: <span className="font-semibold text-zinc-300">{statusLabel(p.status)}</span></span>
